...
- `config/email_config.php`, `config/email_policy.php` - Email configuration and allowed-domain policy.
...
- Password reset links are generated by `auth/forgot_password.php` and routed through `redirects/handle_password_reset_token_redirect.php`.
- Developer/admin utility scripts should stay CLI-only unless explicitly designed for browser use.

*Last updated: current snake_case API tree and renamed endpoint paths.*
