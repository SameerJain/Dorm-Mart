<?php
/**
 * Shared helpers for api-test-files integration scripts.
 *
...
function api_test_api_base_url(): string
{
    ...
    if (!empty($_SERVER['HTTP_HOST']) && !empty($_SERVER['SCRIPT_NAME'])) {
        ...
        // .../api/api-test-files/.../script.php -> .../api
        if (preg_match('#^(.*)/api/api-test-files/#', $script, $m)) {
...
/**
 * POST JSON to an endpoint under api/ (path like "auth/reset-password.php").
 *
