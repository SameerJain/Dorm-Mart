...
try {
    ...
    $stmt = $conn->prepare('SELECT * FROM electronic_payments WHERE electronic_payment_id = ? LIMIT 1 FOR UPDATE');
    ...
    if (($payment['status'] ?? '') === 'refund_pending' && !empty($payment['stripe_refund_id'])) {
        $conn->commit();
        json_response(['success' => true, 'data' => ['status' => 'refund_pending']]);
    }
    if (!payment_refund_needs_request((string)$payment['status'], $payment['stripe_refund_id'] ?? null)) {
        $conn->rollback();
        json_response(['success' => false, 'error' => 'This payment cannot be refunded'], 409);
    }
    if (($payment['status'] ?? '') === 'refund_pending') {
        $relist = (int)($payment['refund_relist'] ?? 0) === 1;
    }
    ...
    if ($refundStatus === 'succeeded') {
        ...
        $failureReason = (string)($refund->failure_reason ?? $refundStatus);
        $failed = $conn->prepare(
            "UPDATE electronic_payments
                SET status = 'refund_failed', stripe_refund_id = ?, last_error_code = ?
              WHERE electronic_payment_id = ? AND status <> 'refunded'"
        );
        if (!$failed) throw new RuntimeException('Failed to prepare refund failure update');
        $failed->bind_param('ssi', $refundId, $failureReason, $paymentId);
        $failed->execute();
        $failed->close();
        $localStatus = 'refund_failed';
    } else {
        $localStatus = 'refund_pending';
    }
    $conn->commit();

    json_response(['success' => true, 'data' => ['status' => $localStatus]]);
} catch (Throwable $e) {
    if (isset($conn)) {
