...
function DeleteAccountPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/profile/account_info.php`, { credentials: "include" })
      .then((response) => response.json())
      .then((data) => {
        if (data?.success && data.account?.email) setAccountEmail(data.account.email);
      })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    if (!isOpen) return undefined;
    setContinueCountdown(CONFIRM_DELAY_SECONDS);
  ...
  useEffect(() => {
    ...
    setDeleteCountdown(CONFIRM_DELAY_SECONDS);
    const interval = setInterval(() => {
  ...
  const canDelete =
    ...
    confirmation.trim().toLowerCase() === accountEmail.trim().toLowerCase() &&
    password.length > 0 &&
    deleteCountdown === 0;
    ...
    <SettingsLayout>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="presentation">
          ...
          <div
            className="relative z-10 flex max-h-[calc(100dvh-2rem)] w-full max-w-lg flex-col overflow-y-auto rounded-2xl border border-gray-200 bg-white p-4 shadow-2xl dark:border-gray-700 dark:bg-gray-800 sm:p-6"
            role="dialog"
            aria-modal="true"
          ...
          >
            {step === 1 ? (
              <>
                <h2 id="delete-account-title" className="font-serif text-xl font-semibold text-red-700 dark:text-red-400 sm:text-2xl">
                  Before you delete your account
                </h2>
                <div className="mt-4 flex gap-3 rounded-xl border-2 border-red-300 bg-red-50 p-4 dark:border-red-800 dark:bg-red-950/30">
                  <svg className="h-6 w-6 flex-shrink-0 text-red-600 dark:text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
                  </svg>
                  <div>
                    <p className="font-semibold text-red-800 dark:text-red-200">This is permanent. Deleting your account means:</p>
                    <ul className="mt-2 list-disc space-y-1.5 pl-5 text-sm text-red-700 dark:text-red-200">
                ...
                <div className="mt-6 flex justify-end gap-3">
                  ...
                  >
                    {continueCountdown > 0 ? `Continue (${continueCountdown})` : "Continue"}
                  </button>
                </div>
              </>
            ) : (
              <>
                ...
                <label htmlFor="delete-confirmation" className="mt-5 block text-sm font-medium text-gray-700 dark:text-gray-200">
                  Type your email <span className="font-bold">{accountEmail || "..."}</span> to confirm
                </label>
                <input
                ...
                <div className="mt-6 flex justify-between gap-3">
                  ...
                  >
                    {isSubmitting
                      ? "Deleting..."
                      : deleteCountdown > 0
                        ? `Delete my account (${deleteCountdown})`
                        : "Delete my account"}
                  </button>
                </div>
              </>
